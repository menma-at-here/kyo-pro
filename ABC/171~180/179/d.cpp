...
int main(){
    ...
    vector<ll> ans(N+1,0);
    ans[0] = 1;
    Segment_Tree tree(ans);

    REP(i,N-1){
        REP(j,K){
            if(i + 1 - sec[j].second >= 0){
                tree.add(i+1, i+2, tree.getsum(i + 1 - sec[j].second, i + 1 - sec[j].first + 1));
            }
            else{
                if(i + 1 - sec[j].first >= 0){
                    tree.add(i+1, i+2, tree.getsum(0, i + 1 - sec[j].first + 1));
                }
                else{
                    continue;
                }
